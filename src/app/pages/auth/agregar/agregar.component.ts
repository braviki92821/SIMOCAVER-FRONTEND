...
export class AgregarComponent implements OnInit {
  ...
  subirpronostico(): void {

    if(!this.validarDatos()){
      return
    }

    this.referencia.forEach( element => {
      let file: HTMLInputElement = <HTMLInputElement> document.getElementById(`upload${element}`)
      let pronostico = {
  ...
  validarDatos(): boolean {
    for(let i = 0; i < 25; i++) {
      let file: HTMLInputElement = <HTMLInputElement> document.getElementById(`upload${i}`)
      if(file.files?.item(0) === null) {
        SwalFire.fire('Mensaje de Error', `No se ha seleccionado un archivo en la fila: ${i}`, 'error')
        return false
      }
    }
    return true
